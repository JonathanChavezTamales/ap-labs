...
var (
	...
	userbase  = make(chan *User)
	kicking    = make(chan message)
	info_user  = make(chan message)
	clients    = make(map[string]*User) // all connected clients
	broadcastM   = make(chan string)  // broadcast messages
	global_messages     = make(chan message) // cast message (uni or multi)
	private_message = make(chan message)
)
...
func messageManager() {
	for {
		...
		case msg := <-private_message:
			go sendPrivate(msg)
		case msg := <-global_messages:
			go sendAll(msg.from, msg.msg, msg.from)
			break
		case msg := <-broadcastM:
		...
		case msg := <-kicking:
			...
			sendAll("irc-server", "["+msg.to+"] was kicked from the server.", msg.to)
...
func handleConn(conn net.Conn) {
	...
	global_messages <- message{from: "irc-server", msg: "New connected user [" + who.name + "]"}
	entering <- &who
	...
	for input.Scan() {
		...
		if data[0][0] == '/' {
			...
			case "/msg":
				...
				private_message <- message{from: who.name, to: data2[0], msg: data2[1]}
			case "/time":
				t := time.Now()
			...
			global_messages <- message{from: who.name, msg: input.Text()}
		}
	}

	if !who.kicked {
		leaving <- &who
		broadcastM <- "[" + who.name + "] left"
...
func sendAll(from string, msg string, notInclude string) {
	for cli := range clients {
		if cli != notInclude {
